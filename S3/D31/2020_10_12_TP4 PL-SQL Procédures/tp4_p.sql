...
SET SERVEROUTPUT ON;
CREATE OR REPLACE PROCEDURE afficheSalEmp(employe NUMBER) IS res NUMBER;
    BEGIN
        SELECT sal FROM emp WHERE empno=employe;
    END afficheSalEmp;
)
...
/*Les paramètres d'une procédure peuvent avoir un mode
...
CREATE OR REPLACE PROCEDURE augmentPourcent(
  employe IN INTEGER, pourcent IN INTEGER, nouvSal OUT INTEGER
)
IS
BEGIN
UPDATE emp
...
CREATE OR REPLACE PROCEDURE revenuAnnuel(
  id NUMBER
) AS salaire INTEGER;
BEGIN
...
--    de l'employé, en lui ajoutant un 13e mois égal au salaire quand son ancienneté est inférieure à 10 ans, égal au 13e mois augmenté d'un pourcentage égal à son ancienneté quand celle-ci est supérieure ou égale à 10 ans. Testez.

CREATE OR REPLACE PROCEDURE revenuAnnuelPromoAnc(
  id NUMBER
) AS salaireSpe INTEGER;
BEGIN
...
PROCEDURE revenuAnnuel(id NUMBER);
END;
/

CREATE OR REPLACE PACKAGE BODY GestionFinanciere IS
PROCEDURE revenuAnnuel(
  id NUMBER
) AS salaire INTEGER;
BEGIN
SELECT sal INTO salaire
...
--15. Donnez à un autre étudiant le droit d'exécuter le package et vérifiez qu'il peut exécuter
--    une procédure.

GRANT execute ON GestionFinanciere TO someone;

--16. Créez la table mes_augmentations(empno : number(5), date_aug : date,
--    montant_aug : number(6,2)) qui permettra de garder un historique des augmentations.
...
--17. Ecrire un déclencheur qui permet de d'alimenter la table mes_augmentations en cas
--    d'augmentation d'un employé.
